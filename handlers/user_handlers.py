...
from aiogram.filters import Command, ExceptionMessageFilter, CommandObject
...
from units.idea import get_random_activity
from fields import fields
from config_data.config import*


router = Router()# Создаем роутер для обработки запросо
...
async def start(message: Message, command: CommandObject, state: FSMContext):
    ...
    if game:
        ...
        await message.answer("✨ Добро пожаловать в игру Морской бой! ✨\n\n"
        "Выберите действие и нажми соответсвующую кнопку, чтобы начать:\n\n"
        "⛵️️  Играть - начать играть\n"
        "👤  Профиль - твой аккаунт\n"
        "🏆  Топ-10 - рейтинг\n"
        "🧠  Идея - новое занятие, если стало скучно играть",
        reply_markup=await main_keyboard(),)

# Обработчик текстового сообщения "⛵️ Играть"
@router.message(F.text == "⛵️ Играть")
async def play(message: Message):
    await message.delete()
    await message.answer("Выберите режим игры", reply_markup=await game_mode_keyboard())

# Обработчик текстового сообщения "👤 Профиль"
@router.message(F.text == "👤 Профиль")
async def profile(message: Message):
    await message.delete()
    user_info = await get_user(message.from_user.id)
    if user_info: # Проверка на существование пользователя
...
async def profile_command(message: Message, command: CommandObject, state: FSMContext):
    user_info = await get_user(message.from_user.id)
    if user_info: # Проверка на существование пользователя
        await message.answer(f"""👤 <b>Профиль</b>
...
# Обработчик текстового сообщения "🧠 Идея"
@router.message(F.text == "🧠 Идея")
async def get_idea(message: Message):
    await message.delete()
    translated_activity = get_random_activity()
    await message.answer(f"💡 Идея: {translated_activity}")
...
# Обработчик текстового сообщения "🏆 Топ"
...
async def top_command(message: Message, command: CommandObject, state: FSMContext):
    top_10 = await get_top_10()
    text = "🏆 <b>Топ 10 игроков</b>\n\n"
    for user in top_10:
        text += f"<b>{user[1]}</b> - <b>{user[3]}</b> очков\n"
...
# Обработчик callback-запроса на игру с ботом
...
# Обработчик callback-запроса на игру с другим игроком и просмотр списка комнат
...
# Обработчик callback-запроса на присоединение к комнате
...
# Обработчик callback-запроса на выбор типа комнаты
...
# Обработчик callback-запроса на создание комнаты
...
# Обработчик callback-запроса на случайное поле
...
# Обработчик callback-запроса на начало игры
...
# Обработчик callback-запроса на выстрел
...
# Обработчик callback-запроса на завершение игры
    ...
    if room:
        ...
        if number_player == room[7]:
            ...
            if datetime.datetime.strptime(room[8], "%d.%m.%Y %H:%M") + datetime.timedelta(
                    minutes=5) < datetime.datetime.strptime(get_now_date(), "%d.%m.%Y %H:%M"):
...
# Обработчик callback-запроса на то если противник сдаётся
...
# Обработчик callback-запроса на отмену комнаты
...
# Обработчик callback-запроса на закрытие сообщения
...
# Обработка ошибок типа "Telegram server says - Bad Request..."
